...
def demo_ambiguity():
    ...
    print("=" * 70)
    print("When input matches multiple equities (e.g., 'ABC' matches both 'ABC' and 'ABCD')")
    print()
    ...
    for inp in test_cases:
        ...
        if result:
            ...
            print(f"  Top match: {top.equity.symbol} (conf={top.confidence:.2f}, ambiguous={top.ambiguous}){ambiguous_marker}")
            if top.ambiguous:
                print(f"  ⚠️  WARNING: Ambiguous match detected!")
                print(f"  Alternative candidates:")
                for r in result[1:]:
                    print(f"    - {r.equity.symbol} (conf={r.confidence:.2f}, ambiguous={r.ambiguous})")
            elif len(result) > 1:
                print(f"  Alternative candidates:")
                for r in result[1:]:
                    print(f"    - {r.equity.symbol} (conf={r.confidence:.2f}, ambiguous={r.ambiguous})")
    ...
    equities3 = [
        RefMasterEquity(symbol="AAA", isin="", cusip="", currency="USD", exchange="NYSE", pricing_source="test"),
        RefMasterEquity(symbol="AAAB", isin="", cusip="", currency="USD", exchange="NASDAQ", pricing_source="test"),
        RefMasterEquity(symbol="AAAC", isin="", cusip="", currency="USD", exchange="AMEX", pricing_source="test"),
    ]
    ...
    agent_narrow = NormalizerAgent(
        equities=equities3,
        thresholds={"ambiguous_low": 0.7, "ambiguous_high": 0.8}
    )
    ...
    print("KEY TAKEAWAYS")
    print("=" * 70)
    print("1. Ambiguity is flagged when multiple results have confidence in 0.6-0.85 range")
    print("2. Exact matches (ISIN, CUSIP, CIK, exact symbol) have high confidence (>0.85) and are NOT ambiguous")
    print("3. Partial matches (substring, exchange-only) have lower confidence and CAN be ambiguous")
